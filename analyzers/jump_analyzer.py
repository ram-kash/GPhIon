...
#         # Calculate new_dt based on actual frame gap
...
#                 self.frame_gap = actual_frame_gap / self._trajectory.dt  # Store as instance attribute
...
#         # Force correct frame times using new_dt
#         # for i in range(self.n_frames):
#         #     self._trajectory[i].time = i * self.new_dt
        
#         # print(f"Corrected frame times with new_dt: {[self._trajectory[i].new_dt for i in range(min(5, self.n_frames))]}")
...
#         # Calculate current time using new_dt
#         current_time = frame_idx * self.new_dt
...
#                 # Track residence time using new_dt
#                 if self.current_residence_start[i] == -1:
...
#         # Log jumps using new_dt for time calculation
#         for i in range(len(self._ag)):
#             prev_site, curr_site = self._previous_sites[i], current_sites[i]
#             if prev_site > 0 and curr_site > 0 and curr_site != prev_site:
...
#         print(f"  Total jumps detected: {len(self.results.all_trans)}")
#         print(f"  Average site occupancies: {self.results.site_occupancies}")
#         print(f"  Used timestep (new_dt): {self.new_dt} ps")
...
#         print(f"  Least Occupied Site: Site_{np.argmin(self.results.site_occupancies)} "
#               f"({np.min(self.results.site_occupancies):.3f} atoms)")
#         print(f"  Timestep used (new_dt): {self.new_dt} ps")
