...
class TracerPropertyAnalyzer:
    ...
    def calculate_tracer_properties(self, diffusion_dim=3, z_ion=1.0):
        ...
        final_displacements = sim_data.displacement[sim_data.diffusing_atom, -1]
        msd_angstrom_sq = np.mean(final_displacements**2)

        # Calculate Tracer Diffusivity
        # Diffusivity = MSD / (2 * dimensions * time)
        tracer_diff = (msd_angstrom_sq * ang2m**2) / (2 * diffusion_dim * sim_data.total_time)
