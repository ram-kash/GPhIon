"""Jump detection and analysis between discovered sites with occupancy tracking - CORRECTED VERSION."""
...
class SiteAnalyzerMDA(AnalysisBase):
    ...
    def __init__(self, diffusing_atom_group, site_coordinates_cart, sim_data=None, **kwargs):
        ...
        # Store simulation data for consistent time calculation
        ...
        if self.sim_data:
            expected_total_time = self.sim_data.nr_steps * self.sim_data.time_step
            print(f"DEBUG: Using sim_data for time calculation")
            print(f"  Expected total time: {expected_total_time:.6e} s")
            print(f"  Time step: {self.sim_data.time_step:.6e} s")
            print(f"  Total steps: {self.sim_data.nr_steps}")
        else:
            print("WARNING: sim_data not provided. Using trajectory time which may be inconsistent.")
    ...
    def _single_frame(self):
        ...
        # CORRECTED: Calculate time using simulation parameters for consistency
        if self.sim_data:
            current_time = frame_idx * self.sim_data.time_step
        else:
            # Fallback to trajectory time (may be inconsistent)
            current_time = self._trajectory.time
            
        # Debug time calculation for first few frames
        if frame_idx < 5:
            traj_time = self._trajectory.time
            print(f"DEBUG Frame {frame_idx}: calculated_time={current_time:.6e}s, traj_time={traj_time:.6e}s")
        ...
        for i, atom_pos in enumerate(self._ag.positions):
            ...
            if min_dist_sq < close_sqrd:
                ...
                # Track residence time (CORRECTED to use consistent time)
        ...
        # Log jumps (CORRECTED to use consistent time)
    ...
    def _conclude(self):
        ...
        if len(self.results.all_trans) > 0 and self.sim_data:
            max_jump_time = np.max(self.results.all_trans[:, 3])
            expected_max_time = self.sim_data.nr_steps * self.sim_data.time_step
            
            print(f"DEBUG: Time validation:")
            print(f"  Max jump time: {max_jump_time:.6e} s")
            print(f"  Expected max time: {expected_max_time:.6e} s")
            
            if max_jump_time > expected_max_time:
                print(f"WARNING: Jump times exceed simulation time! Check time calculation.")
            else:
                print(f"SUCCESS: Jump times are consistent with simulation parameters.")
