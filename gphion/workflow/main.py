...
def load_simulation_data(dump_file, type_to_element, diff_elem, dt_ps, temperature=None):
    ...
    sim_data.nr_atoms = sim_data.nr_diffusing  # Only process diffusing atoms
    sim_data.diffusing_indices = diff_atoms.indices  # Store original indices

    sim_data.nr_steps = len(u.trajectory)
    sim_data.cart_pos = np.zeros((3, sim_data.nr_atoms, sim_data.nr_steps))

    # Only store positions for diffusing atoms
    for i, ts in enumerate(u.trajectory):
        sim_data.cart_pos[:, :, i] = ts.positions[diff_atoms.indices].T
