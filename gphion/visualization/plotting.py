...
def plot_taog_density(density_grid, edges, threshold_ratio=0.1, output_file=None, save_payload_dir: Optional[str] = None):
    """
    ...
    if indices.shape[0] == 0:
        ...
        # Save metadata that nothing was plotted as well
        try:
            save_plot_data(
                name="taog_density_empty",
                payload={
                    "threshold_ratio": float(threshold_ratio),
                    "max_density": float(density_grid.max()),
                },
                base_dir=save_payload_dir or "plot_data",
            )
        except Exception as e:
            print(f"WARNING: Failed to save empty TAOG payload: {e}")
        return
    ...
    # Persist plot payload for later reproduction
    try:
        save_plot_data(
            name="taog_density",
            payload={
                ...
                "params": {"threshold_ratio": float(threshold_ratio)},
            },
            base_dir=save_payload_dir or "plot_data",
        )
    except Exception as e:
        print(f"WARNING: Failed to save TAOG plot payload: {e}")
