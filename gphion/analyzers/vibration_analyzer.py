...
class VibrationAnalyzer(AnalysisBase):
    """Analyzes atomic vibrations to find attempt frequency and vibration amplitude."""

    def __init__(self, *, show_plot: bool = True, save_data: bool = False, output_dir: str | None = None, save_payload_dir: str | None = None):
        """Initialize the VibrationAnalyzer with standard analysis options."""
    ...
    def vibration_properties(self, sim_data, show_pics=None, save_data=None, output_dir=None):
        """
        ...
        # Main Loop over Diffusing Atoms
        box = sim_data.universe.dimensions[:3] if sim_data.universe is not None else None
        for i in range(sim_data.nr_atoms):
            # List to hold amplitudes for the current atom
            current_atom_amplitudes = []

            # Calculate speed (1D velocity on X axis) and vibration amplitudes
            for time in range(1, sim_data.nr_steps):
                dr = sim_data.cart_pos[:, i, time] - sim_data.cart_pos[:, i, time - 1]
                if box is not None:
                    dr = dr - box * np.round(dr / box)
                current_speed = dr[0]
                speed[i, time] = current_speed

                if time == 1:
                    # Start the first amplitude entry
                    current_atom_amplitudes.append(current_speed)
                else:
                    prev_speed = speed[i, time - 1]
                    # If the atom continues in the same direction, add to the current amplitude
                    if np.sign(current_speed) == np.sign(prev_speed) or current_speed == 0:
            ...
            freq_mean[i] = self._meanfreq(speed[i, :], fs)

            # Perform Fourier Transform on the speed
            trans = np.fft.fft(speed[i, :length])
            two_sided = np.abs(trans / length)
            one_sided[i, :] = two_sided[:length // 2 + 1]
        ...
        # Resolve flags
        eff_show = self.show_plot if show_pics is None else show_pics
        eff_save = self.save_data if save_data is None else save_data
        eff_output = output_dir or self.output_dir
        ...
        # Payload saving disabled to avoid extra timestamped directories
        # # Persist payload for re-plotting
        # try:
        #     base_dir = self.save_payload_dir or eff_output or "plot_data"
        #     save_plot_data(
        #         name="vibration_properties",
        #         payload={
        #             "amplitudes": np.asarray(all_amplitudes),
        #             "freq_mean": freq_mean,
        #             "frequencies": f,
        #             "spectrum": sum_freqs,
        #             "smoothed_spectrum": smoothed_spectrum,
        #             "attempt_freq": float(attempt_freq),
        #             "std_attempt_freq": float(std_attempt_freq),
        #             "vibration_amp": float(vibration_amp),
        #             "mean_vib": float(mean_vib),
        #         },
        #         base_dir=base_dir,
        #     )
        # except Exception as e:
        #     print(f"WARNING: Failed to save vibration payload: {e}")
    ...
    def plot_amplitude_histogram_only(self, vibration_data, bins=401, figsize=(10, 6),
